...
public class GenTableService extends BaseService {
	...
	public GenTable getTableFormDb(GenTable genTable){
		// 如果有表名，则获取物理表
		if (org.apache.commons.lang3.StringUtils.isNotBlank(genTable.getName())){
			
			List<GenTable> list = genDataBaseDictDao.findTableList(genTable);
			if (list.size() > 0){
				
				// 如果是新增，初始化表属性
				if (org.apache.commons.lang3.StringUtils.isBlank(genTable.getId())){
					genTable = list.get(0);
					// 设置字段说明
					if (org.apache.commons.lang3.StringUtils.isBlank(genTable.getComments())){
				...
				for (GenTableColumn e : genTable.getColumnList()){
					...
					if (!b){
						e.setDelFlag(BaseEntity.DEL_FLAG_DELETE);
					}
				}
	...
	public void save(GenTable genTable) {
		...
		for (GenTableColumn column : genTable.getColumnList()){
			...
			if (org.apache.commons.lang3.StringUtils.isBlank(column.getId())){
				column.preInsert();
