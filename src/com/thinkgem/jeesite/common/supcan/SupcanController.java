...
public class SupcanController extends BaseController {
	...
	public TreeList treeList(@PathVariable("typeAlias") String typeAlias) {
		...
		for (Method m : clazz.getMethods()){
			...
			if (supCol != null){
				...
				if (org.apache.commons.lang3.StringUtils.isBlank(col.getName())){
					col.setName(org.apache.commons.lang3.StringUtils.uncapitalize(org.apache.commons.lang3.StringUtils.substring(m.getName(), 3)));
				}
				
				// 无分组
				if (org.apache.commons.lang3.StringUtils.isBlank(supCol.groupId())){
					cols.add(col);
				}
				// 有分组
				else{
	...
	private void listToTree(List<Object> colList, Map<String, Group> groupMap, String parentId, Comparator<Object> comparator){
		for (Map.Entry<String, Group> e : groupMap.entrySet()){
			Group g = e.getValue();
			if (org.apache.commons.lang3.StringUtils.equals(parentId, g.getParentId())){
				colList.add(g);
				// 判断是否有子节点，有的话则加进去
